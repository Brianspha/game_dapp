...
import { mainnet, arbitrum, localhost } from 'viem/chains'
import { reconnect } from '@wagmi/core'
import { createClient } from 'viem'
import TOKENABI from "../../contracts/out/Token.sol/Token.json"
import GameABI from "../../contracts/out/Game.sol/Game.json"
import GameConfig from "../../contracts/deploy-out/deploymentSepolia.json"
const gameABI = GameABI.abi
const tokenABI = TOKENABI.abi
const gameAddress = GameConfig.Game
const tokenAddress = GameConfig.Token
...
// 1. Define constants
const projectId = import.meta.env.VITE_WALLET_CONNECT_APID
...
reconnect(config)
// 3. Create modal
const walletConnect = createWeb3Modal({
                ...
                switch (mainMenuOptions.text) {
                    ...
                    case "Play":
                        if (store.state.connected && store.state.canPlay) {
                            ...
                            await connectWallet();
                            console.log("walletInfo: ", getAccount(), store.state.address, " connected: ", store.state.connected)
                            if (store.state.connected) {
                                const success = await paidPlay()
                                if (success) {
                                    hideMenu()
                                    initGame()
                                }
                    ...
                    case "FreePlay":
                        if (store.state.connected && store.state.canPlay) {
                            ...
                            await connectWallet();
                            console.log("walletInfo: ", store.state.address, " connected: ", store.state.connected)
                            if (store.state.connected) {
                                const success = await freePlay()
                                if (success) {
                                    hideMenu()
...
async function freePlay() {
    try {
        ...
        let alreadyClaimed = await readContract(wagmiConfig, {
            abi: GameABI,
        ...
        if (alreadyClaimed) {
            ...
            store.dispatch("toastError", "You have already claimed your free play")
            return false
        }
        ...
        store.state.isLoading = false
        store.dispatch("toastSuccess", "Succesfully claimed free play")
        return true
    }
    catch (error) {
        console.log("error calling free play: ", error)
        store.dispatch("error", "Error calling free play")
        store.state.isLoading = false
        return false
    }
}
async function paidPlay() {
    try {
        store.state.isLoading = true
        const receipt = await writeContract(wagmiConfig, {
            address: gameAddress,
        ...
        console.log("receipt: ", receipt)
        store.state.isLoading = false
        store.dispatch("toastSuccess", "Succesfully Purchased play token")
        return true
    }
    catch (error) {
        console.log("error calling play: ", error)
        store.dispatch("error", "Error Purchasing a play token")
        store.state.isLoading = false
        return false
    }
}
function sortScoresAndAddresses() {
...
async function claimWinnings() {
    ...
    if (store.state.winnings.length === 0) {
        store.dispatch("error", "Not eligble for any winnings")
        return
    }
    try {
        store.state.isLoading = true
        ...
        if (latestScores.length === 0) {
            store.dispatch("error", "Something went wrong")
            return
        }
        ...
        const updatedData = sortScoresAndAddresses([
        ...
        const encodedPacked = keccak256(encodePacked(
            ...
            [userscores, latestAddresses, localhost.id]
        ))
        const signature
        ...
        if (txReceipt.status !== "reverted") {
            store.dispatch("success", "Successfully claimed winnings please check your wallet for latest balances")
        }
        else {
            store.dispatch("error", "Something went wrong while submitting scores")
        }
        store.state.isLoading = false
        return true
    } catch (error) {
...
async function connectWallet() {
    try {
        await open()
        const wallet = walletConnect.getCaipAddress()
        console.log("walletInfo: ", wallet)
        if (wallet && wallet.split(":").length === 3) {
            store.state.address = wallet.split(":")[2]
